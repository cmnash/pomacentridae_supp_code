          ...
          time_axis_ticks, agemax, tip_size, offset) 
{
  ...
  if (tree_layout %in% c("circular", "fan", "radial")) {
    maxx <- ggtree_object$data %>% top_n(n = 1, wt = .data$x) %>% 
      select(.data$x) %>% unlist %>% unname %>% unique
    maxx <- round(maxx, 1)
    ...
    pp <- tibble(x = rev(pretty_points), y = 0) %>% filter(.data$x <= 
                                                             maxx, .data$x > 0) %>% mutate(label = rev(.data$x) - 
                                                                                             min(.data$x))
          ...
          open_angle = 10, right = T, size = 1, tip_size = 1, offset = 0) 
{
  ...
  if (discrete) {
    max_rate <- datas %>% select(.data$wanted) %>% unlist %>% 
      unname %>% max
    if (!0 %in% breaks) {
      breaks <- c(0, breaks)
    }
    ...
    print(breaks)
    param <- datas %>% select(.data$wanted) %>% unlist %>% 
      unname %>% cut(breaks = breaks)
    ggg <- ggg + aes(color = param) + scale_color_manual(values = colors, 
                                                         name = parameter) + guides(color = guide_legend(override.aes = list(size = 4)))
  }
  else {
    param <- datas %>% select(.data$wanted) %>% unlist %>% 
      unname
  ...
  ggg <- tree_flip(ggtree_object = ggg, show_tip_labels = show_tip_labels, 
                   ...
                   time_axis_ticks = time_axis_ticks, agemax = agemax, tip_size = tip_size)
...
m_ridgelines_adjust <- function (processed_recon, states_names = c("00", "01", "10", 
                                            "11"), parameter = "turnover", plot_as_waiting_time = FALSE, 
          fill_colors = rep(NA, 4), line_colors = viridis(n = 4), state_order = c("00", "01", "10", 
                                                                                  "11")) 
{
  ...
  ss <- utilhisse:::m_prep_df(processed_recon = processed_recon, states_names = states_names, 
                  parameter = parameter)
  message("Summarising grouped by character state\n")
  wanted <- as.name(parameter)
  ss_var <- ss %>% group_by(.data$four_state) %>% select(.data$four_state, 
  ...
  max_rate <- ss %>% select(.data$wanted) %>% top_n(1, wt = .data$wanted) %>% 
    ...
    ggridges::geom_density_ridges(alpha = 0.75, size = 0.75, jittered_points = TRUE, 
                        ...
                        point_size = 1, position = position_nudge(y = rep(-0.2, 
                                                                          4))) + geom_errorbarh(data = ss.sum, position = position_nudge(y = rep(-0.3, 
                                                                                                                                                 4)), aes(xmin = .data$Mean - .data$SD, xmax = .data$Mean + 
                                                                                                                                                            .data$SD, y = .data$four_state, colour = .data$four_state), 
                                                                                                height = 0.05, inherit.aes = FALSE) + geom_point(data = ss.sum, 
                                                                                                                                                 ...
                                                                                                                                                 size = 3, inherit.aes = FALSE) + scale_x_continuous(breaks = pretty(x = c(0, 
